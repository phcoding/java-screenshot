...
public class BPanel extends JPanel {
	...
	static boolean mousePower_X=false;
	
	static boolean mousePower_Y=false;
	...
	public BPanel() {
		...
		panel.addMouseMotionListener(new MouseMotionAdapter() {
			...
			public void mouseDragged(MouseEvent e) {
				...
				else if(X+r.getW()>BPanel.this.getWidth()){
					X=BPanel.this.getWidth()-r.getW();
					mousePower_X=false;
				}
				else{
					mousePower_X=true;
					panel.setLocation(X-panel.getDev()/2, r.getY());
					r.setX(X);
				}
				...
				else if(Y+r.getH()>BPanel.this.getHeight()){
					Y=BPanel.this.getHeight()-r.getH();
					mousePower_Y=false;
				}
				else{
					mousePower_Y=true;
					panel.setLocation(r.getX(), Y-panel.getDev()/2);
					r.setY(Y);
				}
			}
		});
